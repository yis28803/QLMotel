...
import { faHome, faUsers, faBed, faConciergeBell, faSignOutAlt } from '@fortawesome/free-solid-svg-icons';
import  { useEffect, useState } from "react";
import { apiLoginAdmin } from "../api";

const AdminHeader = ({ onLogout }) => {
  return (
    <header className='bg-gray-900 text-gray-100 shadow-lg'>
      <div className='container mx-auto flex justify-between items-center p-4'>
        <Link to='/admin' className='flex items-center space-x-2 text-white hover:text-gray-300 transition duration-300 font-semibold'>
        ...
        <nav className='hidden md:flex'>
          <ul className='flex space-x-4'>
            ...
            <li>
              <Link to='/admin/staffs' className='flex items-center space-x-2 text-white hover:text-gray-300 transition duration-300 font-semibold'>
                <FontAwesomeIcon icon={faUsers} />
                <span>Staffs</span>
              </Link>
            </li>
            <li>
              <Link to='/admin/rooms' className='flex items-center space-x-2 text-white hover:text-gray-300 transition duration-300 font-semibold'>
                <FontAwesomeIcon icon={faBed} />
                <span>Rooms</span>
              </Link>
            </li>
            <li>
              <Link to='/admin/services' className='flex items-center space-x-2 text-white hover:text-gray-300 transition duration-300 font-semibold'>
...
const AdminLayout = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await apiLoginAdmin(body);
      ...
      if (response.access_token !== undefined) {
        ...
        setLoginError(false);
        setHasToken(true);
        navigate("/admin", { state: { fullName: response.fullName } });
      } else {
        ...
        <div className='flex flex-col h-screen justify-between'>
          ...
          <div className='flex-grow flex items-center justify-center'>
            <div className='bg-white shadow-xl rounded-lg w-96 p-6 space-y-4'>
              <h1 className='text-3xl font-semibold text-center text-gray-700'>Login Admin</h1>
